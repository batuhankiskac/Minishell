...
/*   Updated: 2025/04/13 13:36:43 by bkiskac          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*
* This function identifies and executes the appropriate built-in command based on
* the command name in the shell structure. It supports the following built-ins:
...
es if (ft_strcmp(shell->command->cmd, "pwd") == 0)
		return (builtin_pwd());
	le if (ft_strcmp(shell->command->cmd, "export") == 0)
		return (builtin_export(shell->command->argc,
				shell->command->args, &shell->env));	ese if (ft_strcmp(shell->command->cmd, "unset") == 0)
		return (builtin_unset(shell->command->argc,
				shell->command->args, &shell->env))
	lse if (ft_strcmp(shell->command->cmd, "env") == 0)
